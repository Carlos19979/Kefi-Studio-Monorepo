  ...
  if (
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.error(
          "Middleware: Error fetching regions from Medusa. Ensure your backend is running and the publishable key is valid.",
          error
        )
      }
    }
...
export const config = {
  matcher: ["/((?!api|_next|favicon.ico|robots.txt|sitemap.xml|.*\\.png|.*\\.jpg|.*\\.gif|.*\\.svg).*)"], // prevents redirecting on static files and Next.js internals
}
